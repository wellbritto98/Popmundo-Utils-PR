...
async function onSubmitClick(submitBtn) {
    ...
    try {
        ...
        if (INTERACTIONS.length === 0) {
            if (statusPElem) statusPElem.textContent = chrome.i18n.getMessage('cafStatusNoInteractions');
            return;
        }
        ...
        // Get exclusion list for the current character from the per-character shard
        const excludedIds = await Utils.getExcludedIds('call_exclude_id', myCharID);
        ...
        for (let friendIndex = 0; friendIndex < friendsInfo.length; friendIndex++) {
            ...
            if (statusPElem) statusPElem.textContent = statusMessage;
        ...
        if (callableFriends.length == 0) {
            if (statusPElem) statusPElem.textContent = chrome.i18n.getMessage('cafStatusNoFriends');
        } else {
            if (statusPElem) statusPElem.textContent = chrome.i18n.getMessage('cafStatusCalling', [String(callableFriends.length), String(friendsInfo.length)]);
            ...
            for (let friendIndex = 0; friendIndex < callableFriends.length; friendIndex++) {
                ...
                if (statusPElem) statusPElem.textContent = chrome.i18n.getMessage('cafStatusCallingOne', [String(friendIndex + 1), String(callableFriends.length), friendDetails.name]);
...
async function injectCallAllHTML() {
    ...
    try {
        ...
        callAllSubmit.onclick = () => { onSubmitClick(callAllSubmit); return false; };
...
async function injectCallAllExcludeButtons() {
    ...
    const { enhanced_links_font_size: fontSize } = await chrome.storage.sync.get({ enhanced_links_font_size: 16 });
    const currentExcludedIds = await Utils.getExcludedIds('call_exclude_id', myCharID);
    ...
    document.addEventListener('click', async (e) => {
        ...
        try {
            const nowExcluded = await Utils.toggleExclusion('call_exclude_id', charKey, friendID, friendName);
